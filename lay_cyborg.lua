...
local hookedNotification; hookedNotification = hookfunction(require(ReplicatedStorage.Notification).new, newcclosure(function(...)
    local args = ({...})[1]
    if CheckSea(2) then
    ...
    while task.wait(0.5) do
        xpcall(function()
            if LocalPlayer.Data.Race.Value ~= "Cyborg" then
                if LocalPlayer.Data.Fragments.Value >= 2500 then COMMF_:InvokeServer("CyborgTrainer", "Buy") end
            ...
            elseif CyborgBlockPartUnlocked == "unlock" or game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CyborgTrainer", "Check") == true then
                ...
                if fragok then
                    if have < 1000 then fragok = false
                    end
                else
                        ...
                        if CheckMonster("Order") then
                            for _, v in next, workspace.Enemies:GetChildren() do
                                if v.Name == "Order" then
                                    if v:FindFirstChildWhichIsA("Humanoid") and v.Humanoid.Health > 0 and v.HumanoidRootPart then
                                        repeat task.wait()
                                        SetText("Killing Law | Health: ".. math.floor(v.Humanoid.Health / v.Humanoid.MaxHealth * 100).. "%")
                                            KillMonster(v.Name)
                                ...
                                for _, v in next, v2:GetChildren() do
                                    if v.Name == "Dough King" or v.Name == "Cake Prince" or v.Name:find("rip_indra") then
                                        if v.Name ~= "rip_indra" then
                                            if not CheckLocation("Dimensional Shift") then
                                                xpcall(function()
                                                    firetouchinterest(LocalPlayer.Character.HumanoidRootPart, workspace.Map.CakeLoaf.BigMirror.Main, 0) task.wait(3)
                                                end, function(e) warn("can't touch mirror: ".. e) end)
                                            end
                                        end
                                        if v:FindFirstChildWhichIsA("Humanoid") and v.Humanoid.Health > 0 and v.HumanoidRootPart then
                                            repeat task.wait()
                ...
                if CheckSea(2) then
                    if CheckTool("Fist of Darkness") then
                        ...
                        if all < getgenv().Settings["Max Chests"] and not CheckTool("Fist of Darkness") then
                            ...
                            if not CheckTool("Fist of Darkness") and not CheckMonster("Darkbeard") then task.wait(5) HopServer() end
                        end
                    end
